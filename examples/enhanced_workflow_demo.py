...
class MockMarcus:
    ...
    def __init__(self):
        # Create persistence
        self.persistence = Persistence(backend=SQLitePersistence(Path("./demo_marcus.db")))
    ...
    async def run_demo(self):
        ...
        await self.report_progress("bob", task_2, "blocked", 50, 
        ...
        await self.complete_task("charlie", task_2_retry, success=True, actual_hours=9.0)
    ...
    async def assign_task(self, agent_id: str, task: Task):
        ...
        print(f"   Predictions: Success={predictions['success_probability']:.1%}, "
        ...
        if context.previous_implementations:
            print(f"   Context: {len(context.previous_implementations)} previous implementations")
        if context.architectural_decisions:
            print(f"   Decisions: {len(context.architectural_decisions)} relevant decisions")
    ...
    async def complete_task(self, agent_id: str, task: Task, success: bool, actual_hours: float):
        ...
        print(f"   ✅ Completed: Success={success}, Time={actual_hours}h, "
              f"Accuracy={outcome.estimation_accuracy:.1%}")
              
    async def report_progress(self, agent_id: str, task: Task, status: str, 
        ...
        await self.events.publish(
            EventTypes.TASK_BLOCKED if status == "blocked" else EventTypes.TASK_PROGRESS,
            agent_id,
            {
                "task_id": task.id,
                "status": status,
