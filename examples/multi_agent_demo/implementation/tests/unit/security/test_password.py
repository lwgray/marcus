...
class TestPasswordHashing:
    ...
    def test_hash_password_creates_valid_hash(self) -> None:
        ...
        # Arrange
        password = "TestPassword123!"  # pragma: allowlist secret  # nosec B105

        # Act
        hashed = hash_password(password)

        # Assert
        assert isinstance(hashed, str)  # nosec B101
        assert len(hashed) == 60  # Bcrypt hashes are always 60 chars  # nosec B101
        assert hashed.startswith("$2b$")  # Bcrypt format identifier  # nosec B101

    def test_hash_password_generates_unique_salts(self) -> None:
        """Test that each hash uses a unique salt."""
        # Arrange
        password = "SamePassword"  # pragma: allowlist secret  # nosec B105
        ...
        # Assert
        assert hash1 != hash2  # Different salts = different hashes  # nosec B101

    def test_hash_password_with_custom_rounds(self) -> None:
        """Test hashing with custom work factor."""
        # Arrange
        password = "TestPassword"  # pragma: allowlist secret  # nosec B105
        ...
        # Assert
        assert "$2b$10$" in hash_10  # nosec B101
        assert "$2b$14$" in hash_14  # nosec B101

    def test_verify_password_correct_password(self) -> None:
        """Test verifying correct password."""
        # Arrange
        password = "CorrectPassword"  # pragma: allowlist secret  # nosec B105
        hashed = hash_password(password)
    ...
    def test_verify_password_incorrect_password(self) -> None:
        ...
        # Arrange
        correct_password = "CorrectPassword"  # pragma: allowlist secret  # nosec B105
        wrong_password = "WrongPassword"  # pragma: allowlist secret  # nosec B105
    ...
    def test_verify_password_invalid_hash(self) -> None:
        ...
        # Assert
        assert result is False  # Should not raise exception  # nosec B101

    def test_verify_password_case_sensitive(self) -> None:
        """Test that password verification is case-sensitive."""
        # Arrange
        password = "Password123"  # pragma: allowlist secret  # nosec B105
        hashed = hash_password(password)
        ...
        # Assert
        assert result_lower is False  # nosec B101
        assert result_upper is False  # nosec B101
...
class TestPasswordStrength:
    ...
    def test_password_strength_very_weak(self) -> None:
        ...
        # Assert
        assert result["score"] == 0  # nosec B101
        assert result["length"] == 5  # nosec B101
        assert "Very weak" in result["recommendation"]  # nosec B101
    ...
    def test_password_strength_weak(self) -> None:
        ...
        # Assert
        assert result["score"] in [1, 2]  # nosec B101
        assert result["has_lower"] is True  # nosec B101
        assert result["has_upper"] is False  # nosec B101
        assert result["has_digit"] is False  # nosec B101
        assert result["has_special"] is False  # nosec B101
    ...
    def test_password_strength_moderate(self) -> None:
        ...
        # Assert
        assert result["score"] >= 2  # nosec B101
        assert result["has_lower"] is True  # nosec B101
        assert result["has_upper"] is True  # nosec B101
        assert result["has_digit"] is True  # nosec B101
    ...
    def test_password_strength_good(self) -> None:
        ...
        # Assert
        assert result["score"] >= 4  # nosec B101
        assert result["has_lower"] is True  # nosec B101
        assert result["has_upper"] is True  # nosec B101
        assert result["has_digit"] is True  # nosec B101
        assert result["has_special"] is True  # nosec B101
    ...
    def test_password_strength_strong(self) -> None:
        ...
        # Assert
        assert result["score"] == 5  # nosec B101
        assert result["has_lower"] is True  # nosec B101
        assert result["has_upper"] is True  # nosec B101
        assert result["has_digit"] is True  # nosec B101
        assert result["has_special"] is True  # nosec B101
        assert result["recommendation"] == "Strong"  # nosec B101
    ...
    def test_password_strength_only_special_chars(self) -> None:
        ...
        # Assert
        assert result["has_special"] is True  # nosec B101
        assert result["has_lower"] is False  # nosec B101
        assert result["has_upper"] is False  # nosec B101
        assert result["has_digit"] is False  # nosec B101
