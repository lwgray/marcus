...
async def demo_natural_language() -> None:
    ...
    result = await parser.parse_prd_to_tasks(
        simple_project, ProjectConstraints(team_size=2)
    )
    ...
    print(
        f"⏱️  Estimated timeline: {result2.estimated_timeline['estimated_duration_days']} days"
    )
    ...
    print(
        "   Marcus: 'Complete: API development (24h), Frontend integration (16h), Testing (8h)'"
    )
