...
class TodoAppDemo:
    ...
    async def demonstrate_smart_dependency_detection(self, tasks):
        ...
        print("-" * 50)
        print(
            "Marcus analyzes the tasks and automatically detects logical dependencies..."
        )

        # Let Marcus analyze dependencies
        dependency_map = await self.context.analyze_dependencies(
            tasks, infer_implicit=True
        )
        ...
        print(
            f"   situations like 'trying to build the frontend before the API exists'"
        )
    ...
    async def demonstrate_intelligent_task_assignment(self, ordered_tasks):
        ...
        for agent in agents:
            ...
            print(
                f"   {agent}: {success_prob:.0f}% success, {duration:.1f}h duration (confidence: {confidence:.0f}%)"
            )
    ...
    async def demonstrate_context_sharing(self):
        ...
        for i, decision in enumerate(decisions, 1):
            await self.context.log_decision(
                agent_id="alice", task_id="task_1", **decision
            )
    ...
    async def demonstrate_real_time_visibility(self):
        ...
        events_to_simulate = [
            (
                "Bob starts authentication work",
        ...
        # Show basic visibility insights
        ...
        print(
            f"   • Blockers resolved: {stats['event_counts'].get('blocker_resolved', 0)}"
        )
        print(
            f"   • Context updates: {stats['event_counts'].get('context_updated', 0)}"
        )
    ...
    async def demonstrate_integrated_visibility(self):
        """Show how visibility integrates with all enhanced systems"""
        print(f"\n🔗 INTEGRATED SYSTEM VISIBILITY")
        print("-" * 50)
        print(
            "Now see how visibility connects Context, Memory, and Dependency systems..."
        )

        # Get comprehensive insights from all systems
        print(f"\n🧠 Context System Insights:")
        context_insights = await self.visualizer.get_context_insights()
        if "error" not in context_insights:
            health = context_insights["system_health"]
            flow = context_insights["knowledge_flow"]
            print(f"   • Implementations tracked: {health['total_implementations']}")
            print(f"   • Decisions logged: {health['total_decisions']}")
            print(f"   • Knowledge sharing active: {flow['context_sharing_active']}")
            print(f"   • Cross-task references: {flow['cross_task_references']}")

        print(f"\n📊 Memory System Intelligence:")
        memory_insights = await self.visualizer.get_memory_predictions()
        if "error" not in memory_insights:
            intel = memory_insights["system_intelligence"]
            profiles = memory_insights["agent_profiles"]
            print(f"   • Task outcomes tracked: {intel['total_outcomes_tracked']}")
            print(f"   • Agents profiled: {intel['total_agents_profiled']}")
            print(
                f"   • Enhanced predictions available: {intel['prediction_confidence_available']}"
            )
            print(f"   • Active learning: {intel['learning_active']}")
            ...
            if profiles:
                ...
                for agent_id, profile in list(profiles.items())[:3]:
                    if "success_rate" in profile:
                        print(
                            f"     - {agent_id}: {profile['success_rate']*100:.0f}% success rate"
                        )

        print(f"\n🔗 System Integration Health:")
        correlations = await self.visualizer.get_system_correlations()
        if "error" not in correlations:
            health = correlations["integration_health"]
            synergy = correlations["system_synergy"]
            print(
                f"   • Systems connected: {health['systems_connected']}/{health['total_systems']}"
            )
            print(
                f"   • Integration completeness: {health['integration_completeness']*100:.0f}%"
            )
            print(f"   • Real-time intelligence: {synergy['real_time_intelligence']}")
            print(f"   • Context-Memory synergy: {synergy['context_memory_synergy']}")

            # Show value amplification if available
            if "value_amplification" in correlations:
                amp = correlations["value_amplification"]
                print(
                    f"   • Intelligence multiplier: {amp['intelligence_multiplier']:.1f}x"
                )
                print(f"   • Enhanced events: {amp['enhanced_system_events']}")

        print(f"\n🎯 What This Means:")
        print(f"   ✓ All systems working together seamlessly")
        print(f"   ✓ Real-time insights from multiple intelligence sources")
        print(f"   ✓ Project health visible at a glance")
        print(f"   ✓ Problems predicted before they become critical")
        print(f"   ✓ Team performance optimized through data-driven insights")

        # Show enhanced dashboard data
        print(f"\n📈 Enhanced Dashboard Summary:")
        dashboard = self.visualizer.get_enhanced_dashboard_data()
        integrations = dashboard["enhanced_integrations"]
        print(f"   • Systems integrated: {integrations['systems_integrated']}")
        print(f"   • Active monitoring flows: {dashboard['active_flows']}")
        print(f"   • Last updated: {dashboard['last_updated'][:19]}")

        print(f"\n💡 Marcus Insight: This is the difference between tracking work")
        print(
            f"   and intelligently orchestrating it. Visibility + Intelligence = Success!"
        )
...
async def main():
    ...
    await demo.demonstrate_integrated_visibility()  # NEW: Show system integration
