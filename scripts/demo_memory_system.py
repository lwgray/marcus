    ...
    for agent_id, profile in agents.items():
        ...
        for i in range(20):
            ...
            if not success and random.random() < 0.5:
                blockers = [random.choice(["unclear_requirements", "technical_debt", 
                                         "missing_dependency", "api_changes"])]
...
async def demonstrate_basic_predictions(memory: MemoryEnhanced):
    ...
    print("-" * 80)
    print(f"{'Agent':<10} {'Task':<30} {'Success':<10} {'Duration':<12} {'Suitable':<10}")
    print("-" * 80)
    ...
    for task in test_tasks:
        ...
        for agent_id in agents:
            ...
            suitable = prediction["is_suitable_agent"]
            
            print(f"{agent_id:<10} {task.name[:28]:<30} "
                  f"{success_prob*100:<9.0f}% {duration:<11.1f}h "
                  f"{'Yes' if suitable else 'No':<10}")
...
async def demonstrate_enhanced_predictions(memory: MemoryEnhanced):
    ...
    for agent_id in ["alice", "bob", "charlie"]:
        ...
        print(f"Duration Range: {ci['duration_lower']:.1f} - {ci['duration_upper']:.1f} hours")
        print(f"Success Range: {ci['success_lower']*100:.0f}% - {ci['success_upper']*100:.0f}%")
        ...
        for risk in predictions['risk_analysis']['factors'][:3]:
            ...
            print(f"    Impact: {risk['impact']} | Likelihood: {risk['likelihood']*100:.0f}%")
        
        print(f"\nOverall Risk Level: {predictions['risk_analysis']['overall_risk']}")
...
async def demonstrate_learning_progress(memory: MemoryEnhanced, events: Events):
    ...
    print(f"  Success: {updated_pred['success_probability']*100:.0f}% "
          f"(+{(updated_pred['success_probability'] - initial_pred['success_probability'])*100:.0f}%)")
    print(f"  Confidence: {updated_pred['confidence']*100:.0f}% "
    ...
    print(f"  Success rate: {profile['success_rate']*100:.0f}%")
    print(f"  Top skills: {', '.join(profile['expertise'][:3])}")
...
async def demonstrate_pattern_detection(memory: MemoryEnhanced):
    ...
    if failure_patterns:
        for blocker, count in sorted(failure_patterns.items(), 
    ...
    estimation_data = []
    for outcome in memory.episodic.get("outcomes", []):
        if outcome["success"] and "task_estimated_hours" in outcome:
            estimated = outcome["task_estimated_hours"]
    ...
    if estimation_data:
        ...
        if complex:
            print(f"   Complex tasks (>10h): {sum(complex)/len(complex):.2f}x estimated")
    ...
    for outcome in memory.episodic.get("outcomes", []):
        if outcome["success"]:
            ...
            labels = outcome.get("task_labels", [])
            for label in labels:
                key = (agent_id, label)
                agent_strengths[key] = agent_strengths.get(key, 0) + 1
    
    # Group by agent
    agents_analyzed = set()
    for (agent_id, label), count in sorted(agent_strengths.items(), 
                                          key=lambda x: x[1], reverse=True):
        if agent_id not in agents_analyzed and count >= 3:
            agents_analyzed.add(agent_id)
            print(f"\n   {agent_id}:")
            # Get top skills for this agent
            agent_skills = [(l, c) for (a, l), c in agent_strengths.items() 
                           if a == agent_id]
            for skill, skill_count in sorted(agent_skills, 
                                           key=lambda x: x[1], reverse=True)[:3]:
                print(f"     - {skill}: {skill_count} successful tasks")
