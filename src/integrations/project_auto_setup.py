...
class ProjectAutoSetup:
    ...
    async def setup_new_project(
        ...
        if provider == "planka":
            ...
            raise ValueError(
                ...
                f"Supported providers: planka, github, linear, sqlite"
            )
    ...
    async def setup_sqlite_project(
        self,
        project_name: str,
        options: Optional[Dict[str, Any]] = None,
        kanban_client: Any = None,
    ) -> ProjectConfig:
        """Create a new SQLite-backed project scope.

        Generates unique project_id and board_id so multiple
        experiments can share one SQLite database file.
        ...
        options : Dict[str, Any], optional
            ...
            - attachments_dir: Path for attachments (default from config)
            - project_root: Directory where agents write code
        kanban_client : Any, optional
            SQLiteKanban instance to call auto_setup_project on.

        Returns
        -------
        ProjectConfig
            Project configuration for the SQLite provider.
        """
        if options is None:
            options = {}
        ...
        db_path = options.get(
            "db_path",
            config.kanban.sqlite_db_path or "./data/kanban.db",
        )
        attachments_dir = options.get(
            "attachments_dir",
            config.kanban.sqlite_attachments_dir or "./data/attachments",
        )

        logger.info(f"Setting up SQLite project '{project_name}' at {db_path}")

        # Generate project/board IDs via kanban client
        project_id = None
        board_id = None
        if kanban_client and hasattr(kanban_client, "auto_setup_project"):
            result = await kanban_client.auto_setup_project(
                project_name=project_name,
            ...
            board_id = result["board_id"]
            logger.info(
                f"Created SQLite project scope: "
