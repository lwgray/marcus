...
class EnhancedTaskClassifier:
    ...
    TASK_KEYWORDS = {
        TaskType.DESIGN: {
            'primary': [
                ...
                'blueprint', 'specification', 'spec', 'specs'
            ],
            'secondary': [
                'wireframe', 'mockup', 'prototype', 'diagram', 'model',
                'schema', 'structure', 'layout', 'interface', 'ui/ux',
                'ux', 'ui', 'flow', 'workflow', 'concept', 'draft',
                'outline', 'framework', 'pattern', 'template'
            ],
            'verbs': [
        ...
        TaskType.IMPLEMENTATION: {
            'primary': [
                ...
                'create', 'construct', 'engineer'
            ],
            'secondary': [
                'feature', 'functionality', 'component', 'module',
                'service', 'api', 'endpoint', 'integration', 'backend',
                'frontend', 'database', 'logic', 'algorithm', 'function',
                'method', 'class', 'handler', 'controller', 'middleware'
            ],
            'verbs': [
                'implement', 'build', 'create', 'develop', 'code',
                'write', 'add', 'integrate', 'setup', 'configure',
                'establish', 'construct', 'generate', 'produce'
            ]
        },
        TaskType.TESTING: {
            'primary': [
                'test', 'testing', 'qa', 'quality', 'verify',
                'validate', 'check', 'assert'
            ],
            'secondary': [
                'unit', 'integration', 'e2e', 'end-to-end', 'functional',
                'regression', 'smoke', 'acceptance', 'performance',
                'load', 'stress', 'coverage', 'suite', 'scenario',
                'case', 'cases', 'spec', 'specification', 'behavior'
            ],
            'verbs': [
                'test', 'verify', 'validate', 'check', 'ensure',
                'confirm', 'assert', 'examine', 'inspect', 'audit'
            ]
        },
        TaskType.DOCUMENTATION: {
            ...
            'secondary': [
                'tutorial', 'howto', 'how-to', 'reference', 'api-docs',
                'changelog', 'notes', 'instructions', 'help', 'faq',
                'examples', 'samples', 'comments', 'annotations',
                'description', 'explanation'
            ],
            'verbs': [
                'document', 'write', 'create', 'update', 'maintain',
                'annotate', 'comment', 'describe', 'explain', 'detail'
            ]
        },
        TaskType.DEPLOYMENT: {
            'primary': [
            ...
            'secondary': [
                'production', 'staging', 'live', 'go-live', 'ship',
                'deliver', 'distribution', 'installation', 'setup',
                'migration', 'upgrade', 'rollback', 'hotfix'
            ],
            'verbs': [
                'deploy', 'release', 'launch', 'publish', 'ship',
                'deliver', 'distribute', 'install', 'migrate'
            ]
        },
        TaskType.INFRASTRUCTURE: {
            'primary': [
                'infrastructure', 'setup', 'configure', 'provision',
                'environment', 'devops'
            ],
            'secondary': [
                'server', 'database', 'network', 'docker', 'kubernetes',
                'k8s', 'container', 'vm', 'cloud', 'aws', 'azure',
                'gcp', 'ci/cd', 'pipeline', 'monitoring', 'logging',
                'security', 'firewall', 'ssl', 'dns', 'cdn'
            ],
            'verbs': [
                'setup', 'configure', 'provision', 'install',
                'initialize', 'bootstrap', 'orchestrate', 'manage'
            ]
        }
    }
    ...
    TASK_PATTERNS = {
        ...
        TaskType.IMPLEMENTATION: [
            r'(?:implement|build|create)\s+(?:the\s+)?(?:\w+\s+)?(?:feature|functionality|component)',
            r'(?:add|integrate)\s+(?:\w+\s+)?(?:support|integration)\s+(?:for|with)',
            r'(?:develop|code|write)\s+(?:the\s+)?(?:\w+\s+)?(?:api|service|endpoint)',
    ...
    def _score_task_type(self, text: str, task_type: TaskType) -> Tuple[float, List[str], List[str]]:
        """
    ...
    def _generate_reasoning(self, task_type: TaskType, keywords: List[str], patterns: List[str]) -> str:
        ...
        if keywords:
            primary_keywords = [k for k in keywords if k in self.TASK_KEYWORDS[task_type].get('primary', [])]
            if primary_keywords:
                reasons.append(f"contains primary keywords: {', '.join(primary_keywords[:3])}")
            else:
