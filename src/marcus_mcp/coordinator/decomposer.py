...
"""
    ...
    # #557 Fix 1: ground subtask acceptance criteria in the parent task's
    # criteria. For contract_first runs the parent's acceptance_criteria
    # were generated by decompose_by_contract with the domain contract in
    # hand — so partitioning them across subtasks keeps subtask criteria
    # traceable to the contract instead of being re-invented blind.
    parent_criteria = getattr(task, "acceptance_criteria", None) or []
    if parent_criteria:
        prompt += (
            "\n\n**Parent Acceptance Criteria (authoritative):**\n"
            + "\n".join(f"- {c}" for c in parent_criteria)
            + "\n\nEach subtask's `acceptance_criteria` MUST be drawn from "
            "this list — partition and refine these parent criteria across "
            "the subtasks so that together the subtasks cover all of them. "
            "Do NOT invent acceptance criteria that are absent from, or "
            "contradict, this list.\n"
        )
...
always provide specific, verifiable criteria. **If a "Parent
Acceptance Criteria" list is given above, the subtask criteria must be
drawn from it — partition those parent criteria across the subtasks
