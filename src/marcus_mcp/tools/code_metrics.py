    ...
    if agent_data:
        ...
        for project_id, context in state._project_contexts.items():
            if context.kanban_provider:
                ...
                # Rough estimates
                completed_tasks = len([t for t in agent_tasks if str(t.status) == "done"])
                metrics["commits"] = completed_tasks * 3  # Assume 3 commits per task
                metrics["lines_added"] = completed_tasks * 150  # Assume 150 lines per task
                ...
                for task in agent_tasks:
                    for label in (task.labels or []):
                        if label in ["python", "javascript", "typescript", "go", "rust"]:
                            metrics["languages"][label] = metrics["languages"].get(label, 0) + 1

    # Convert files_changed to count
    metrics["files_changed"] = len(set(metrics["files_changed"]))
