...
class FilePersistence(PersistenceBackend):
    ...
    def __init__(self, storage_dir: Path = None):
        ...
        self.storage_dir = storage_dir or Path("./data/marcus_state")
    ...
    async def query(self, collection: str, filter_func=None, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        async with lock:
            ...
            try:
                ...
                items = [
                    {"_key": k, **v} for k, v in data.items()
                ]
        ...
        def _query():
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.execute("""
                    ...
                    LIMIT ?
                """, (collection, limit * 2))  # Get extra for filtering
...
class Persistence:
    ...
    async def get_decisions(self, task_id: Optional[str] = None,
