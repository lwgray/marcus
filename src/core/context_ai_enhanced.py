...
class ContextAIEnhanced(Context):
                ...
                for task_id, dependents in ai_dependencies.items():
                    ...
                    for dependent_id in dependents:
                        if dependent_id not in dependency_map[task_id]:
                            ...
                            logger.info(f"AI inferred: Task {dependent_id} depends on {task_id}")
                            
            except Exception as e:
                logger.warning(f"AI dependency inference failed, using pattern-based fallback: {e}")
    ...
    async def _infer_dependencies_with_ai(self, tasks: List[Task]) -> Dict[str, List[str]]:
        """
...
Focus on the most important and logical dependencies. Do not create unnecessary dependencies."""
        ...
        try:
            ...
            for dep in result.get("dependencies", []):
                ...
                if confidence >= 0.7:
                    ...
                    logger.debug(f"AI dependency: {dep['reasoning']} (confidence: {confidence})")
        ...
        for i, task in enumerate(tasks):
            for j, other_task in enumerate(tasks):
                ...
                if "test" in task_name_lower and not "test" in other_name_lower:
                    if any(word in other_name_lower for word in ["implement", "build", "create"]):
    ...
    async def get_dependency_explanation(self, task_id: str, dependency_id: str) -> Optional[str]:
        """
