...
from src.core.resilience import with_retry, with_fallback, RetryConfig
...
class HybridDependencyInferer(DependencyInferer):
    ...
    def __init__(self, ai_engine: Optional[AIAnalysisEngine] = None, 
        ...
        # Log configuration
        logger.info(f"Hybrid inference initialized with config: "
    ...
    async def infer_dependencies(self, tasks: List[Task]) -> DependencyGraph:
        ...
        # Log statistics
        self._log_inference_stats(pattern_dependencies, ai_dependencies, final_dependencies)
        
        return graph
        
    async def _get_pattern_dependencies(self, tasks: List[Task]) -> Dict[Tuple[str, str], HybridDependency]:
            ...
            for dependency_task in tasks:
                ...
                for pattern in self.dependency_patterns:
                    ...
                    if dep:
                        ...
                        if key not in dependencies or dependencies[key].confidence < hybrid_dep.confidence:
    ...
    async def _identify_ambiguous_pairs(
        ...
        for i, task1 in enumerate(tasks):
            for j, task2 in enumerate(tasks):
                ...
                if key not in pattern_dependencies and reverse_key not in pattern_dependencies:
                ...
                # Case 2: Low confidence pattern match
                    ...
                    if pattern_dependencies[key].confidence < self.config.pattern_confidence_threshold:
                        ambiguous_pairs.append((task1, task2))
                        
                # Case 3: Bidirectional dependencies (conflict)
                elif key in pattern_dependencies and reverse_key in pattern_dependencies:
                    ambiguous_pairs.append((task1, task2))
    ...
    def _extract_keywords(self, task: Task) -> List[str]:
        ...
        stop_words = {
            ...
            'create', 'build', 'implement', 'add', 'update', 'fix', 'test'
        }
    ...
    async def _get_ai_dependencies(
        self, 
        tasks: List[Task], 
        ambiguous_pairs: List[Tuple[Task, Task]]
...
"""
    ...
    async def _combine_dependencies(
        ...
        for key, pattern_dep in pattern_deps.items():
            if key in ai_deps:
                ...
                combined_confidence = min(1.0, 
                    (pattern_dep.confidence + ai_dep.ai_confidence) / 2 + self.config.combined_confidence_boost
                )
        ...
        for key, ai_dep in ai_deps.items():
            if key not in combined and ai_dep.confidence >= self.config.ai_confidence_threshold:
                combined[key] = ai_dep
    ...
    def _build_dependency_graph(self, tasks: List[Task], dependencies: List[HybridDependency]) -> DependencyGraph:
...
""")
    ...
    async def explain_dependency(self, dependent_id: str, dependency_id: str, graph: DependencyGraph) -> str:
        """
        ...
        for dep in graph.edges:
            if dep.dependent_task_id == dependent_id and dep.dependency_task_id == dependency_id:
