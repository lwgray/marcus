...
class LivePipelineMonitor:
    ...
    def _load_historical_data(self) -> Dict[str, Any]:
        ...
        for flow_id, flow_info in all_data["flows"].items():
            if flow_info.get("completed_at"):
                flow_events = [e for e in all_data["events"] if e.get("flow_id") == flow_id]
                
                # Track stage durations
                for event in flow_events:
                    if "duration_ms" in event:
                        stage = event.get("stage", "unknown")
                        historical["avg_durations_by_stage"][stage].append(event["duration_ms"])
                ...
                for flow_info in active_flows:
                    ...
                    if flow_id not in self.active_flows:
                        self.active_flows[flow_id] = {
                            "start_time": datetime.fromisoformat(flow_info["started_at"]),
    ...
    def calculate_progress(self, flow_id: str, events: List[Dict[str, Any]]) -> float:
        ...
        expected_stages = [
            "mcp_request", "ai_analysis", "prd_parsing", 
        ...
        avg_events = statistics.mean(self.historical_data["avg_events_per_flow"]) if \
                    self.historical_data["avg_events_per_flow"] else 20
        event_progress = min(len(events) / avg_events * 100, 95)
        
        # Weight both factors
        return min((stage_progress + event_progress) / 2, 95)
        
    def estimate_completion(self, flow_id: str, events: List[Dict[str, Any]], 
        ...
        if current_progress > 0:
            ...
            if current_stage and current_stage in self.historical_data["avg_durations_by_stage"]:
    ...
    def _get_remaining_stages(self, current_stage: str) -> List[str]:
        ...
        stage_order = [
            "mcp_request", "ai_analysis", "prd_parsing",
            "task_generation", "task_creation", "task_assignment",
            "work_progress", "task_completion"
        ]
    ...
    def _get_current_alerts(self) -> List[Dict[str, Any]]:
        ...
        critical_count = sum(
            1 for update in getattr(self, 'last_updates', {}).values()
