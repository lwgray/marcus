...
class HybridDecisionFramework:
    ...
    async def make_assignment_decision(
        ...
        ai_result = None
        try:
            ai_result = await self.ai_engine.analyze_assignment_optimality(
                task, context
            )
        except Exception as e:
            logger.warning(
                f"AI optimization failed, proceeding with rule-based decision: {e}"
            )

        # Step 4: Calculate hybrid confidence
        final_confidence = self._calculate_hybrid_confidence(
            rule_result.confidence, ai_result.confidence if ai_result else None
        )
    ...
    def _calculate_hybrid_confidence(
        ...
        hybrid_confidence = (
            rule_confidence * self.rule_weight + ai_confidence * self.ai_weight
        )
    ...
    async def evaluate_assignment_quality(
        ...
        if estimated_time > 0:
            time_accuracy = min(estimated_time, actual_time) / max(
                estimated_time, actual_time
            )
            evaluation["time_estimation_accuracy"] = time_accuracy

        # Evaluate decision quality
        if assignment_outcome.get("success", True) and not assignment_outcome.get(
            "blockers"
        ):
        ...
        if time_accuracy is not None and time_accuracy < 0.7:
            learnings.append("Improve time estimation accuracy")

        evaluation["learnings"] = learnings

        logger.debug(
            f"Assignment quality for {task.id}: " f"{evaluation['decision_quality']}"
        )
    ...
    async def get_framework_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "components": {"rule_engine": "active", "ai_engine": "active"},
        }
    ...
    async def adjust_weights(self, rule_weight: float, ai_weight: float) -> bool:
        ...
        if rule_weight < 0.5:
            logger.warning(
                f"Rejected weight adjustment: rule weight {rule_weight} < 0.5 minimum"
            )
            return False
        ...
        logger.info(
            f"Adjusted weights: rule={self.rule_weight:.2f}, ai={self.ai_weight:.2f}"
        )
        return True
