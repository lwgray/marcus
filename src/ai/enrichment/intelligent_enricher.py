...
class IntelligentTaskEnricher:
    ...
    async def enrich_task_batch(
        ...
        for i, task in enumerate(tasks):
            ...
            if results:
                enriched_context.existing_tasks.extend(
                    [r.original_task for r in results]
                )
    ...
    async def _generate_intelligent_labels(
        ...
        for component in self.standard_labels["component"]:
            if (
                component in task_intent
                or component in f"{task.name} {task.description or ''}".lower()
            ):
    ...
    async def _estimate_effort_with_ai(
        ...
        try:
            return await self.llm_client.estimate_effort_intelligently(
                task, context_dict
            )
        except Exception as e:
    ...
    def _track_changes(
        ...
        if enhanced_description != original_task.description:
            changes["description"] = {
                ...
                "length_change": len(enhanced_description)
                - len(original_task.description or ""),
            }
    ...
    async def get_enrichment_statistics(
        ...
        enhanced_descriptions = sum(
            1 for r in results if "description" in r.changes_made
        )
        ...
        added_criteria = sum(
            1 for r in results if "acceptance_criteria" in r.changes_made
        )
        ...
        return {
            ...
            "total_labels_added": sum(
                len(r.changes_made.get("labels", {}).get("added", [])) for r in results
            ),
