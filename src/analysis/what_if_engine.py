...
from src.visualization.shared_pipeline_events import SharedPipelineVisualizer, PipelineStage
...
class WhatIfAnalysisEngine:
    ...
    async def simulate_variation(self, modifications: List[PipelineModification]) -> Dict[str, Any]:
        """
        ...
        simulated_flow = await self._simulate_flow(variation_flow_id, modified_params, modifications)
    ...
    async def _simulate_flow(self, flow_id: str, parameters: Dict[str, Any], 
                            modifications: List[PipelineModification]) -> Dict[str, Any]:
        ...
        for mod in modifications:
            if mod.parameter == "team_size":
                ...
                if mod.new_value > mod.original_value:
                    simulated_metrics["task_count"] = int(simulated_metrics["task_count"] * 1.2)
                    simulated_metrics["complexity_score"] *= 1.1
                else:
                    simulated_metrics["task_count"] = int(simulated_metrics["task_count"] * 0.9)
                ...
                if mod.new_value == "minimal":
                    simulated_metrics["task_count"] = int(simulated_metrics["task_count"] * 0.6)
                    simulated_metrics["complexity_score"] *= 0.7
                elif mod.new_value == "detailed":
                    simulated_metrics["task_count"] = int(simulated_metrics["task_count"] * 1.5)
                ...
                if mod.new_value and not mod.original_value:
                    simulated_metrics["task_count"] += int(simulated_metrics["task_count"] * 0.3)
                    simulated_metrics["quality_score"] = min(1.0, simulated_metrics["quality_score"] * 1.2)
    ...
    def compare_flows(self, flow_a: Dict[str, Any], flow_b: Dict[str, Any]) -> ComparisonResult:
        """
        ...
        # Compare decisions
        decision_diffs = self._compare_decisions(flow_a["decisions"], flow_b["decisions"])
        ...
        improvement_summary = " | ".join(improvements) if improvements else "No significant improvements"
    ...
    def _compare_decisions(self, decisions_a: List[Dict[str, Any]], 
                          decisions_b: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
            ...
            if i < len(decisions_a):
                ...
                if dec_a["decision"] != dec_b["decision"]:
                    differences.append({
                        ...
                        "confidence_change": dec_b["confidence"] - dec_a["confidence"]
                    })
            else:
                differences.append({
                    "original": None,
    ...
    def _identify_tradeoffs(self, comparison: ComparisonResult) -> List[str]:
        ...
        if comparison.quality_diff < 0:
            tradeoffs.append(f"Quality reduced by {-comparison.quality_diff * 100:.1f}%")
        if comparison.cost_diff > 0:
            tradeoffs.append(f"Cost increased by ${comparison.cost_diff:.2f}")
        if comparison.complexity_diff > 0:
            tradeoffs.append(f"Complexity increased by {comparison.complexity_diff * 100:.1f}%")
