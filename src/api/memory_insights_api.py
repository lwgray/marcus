    ...
    try:
        ...
        for agent_id, profile in server.memory.semantic["agent_profiles"].items():
            profile_data = {
                ...
                "performance_trend": _calculate_performance_trend(agent_id, server.memory)
            }
...
async def get_agent_history(agent_id: str):
    ...
    try:
        ...
        outcomes = [
            outcome for outcome in server.memory.episodic["outcomes"]
        ...
        if history:
            accuracies = [h["estimation_accuracy"] for h in history if h["estimation_accuracy"] > 0]
...
async def get_task_prediction(agent_id: str, task_id: str):
    ...
    try:
        ...
        if hasattr(server.memory, 'predict_task_outcome_v2'):
            ...
            enhanced_memory = MemoryEnhanced(
                events=server.memory.events,
                persistence=server.memory.persistence
            )
...
async def get_learning_curve():
    ...
    try:
        ...
        return jsonify({
            ...
            "total_weeks": len(learning_curve)
        })
    ...
    try:
        ...
        for outcome in server.memory.episodic["outcomes"]:
            for blocker in outcome.blockers:
                ...
                blocker_impacts[blocker].append(outcome.actual_hours - outcome.estimated_hours)
                
        # Format blocker insights
        insights = []
        for blocker, count in blocker_counts.items():
            avg_impact = statistics.mean(blocker_impacts[blocker]) if blocker_impacts[blocker] else 0
            insights.append({
                "blocker": blocker,
                "occurrences": count,
...
def _calculate_performance_trend(agent_id: str, memory) -> str:
    ...
    recent_outcomes = [
        o for o in memory.episodic["outcomes"]
