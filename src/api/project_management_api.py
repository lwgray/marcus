...
def create_project():
    ...
    try:
        ...
        try:
            ...
            print(f"📝 Creating project '{marcus_params['project_name']}' with description length: {len(marcus_params['description'])} chars")
        ...
        if not marcus_result or 'error' in marcus_result:
            return jsonify({
                'success': False,
                'error': marcus_result.get('error', 'Failed to create project in Marcus')
            }), 500
        ...
        flow_manager.add_event(flow_id, {
...
def add_feature():
    ...
    try:
        ...
        # Add event to flow
        if project['flow_id']:
            flow_manager.add_event(project['flow_id'], {
                'type': 'feature_added',
