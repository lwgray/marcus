...
class BoardQualityValidator:
    ...
    def _validate_description(self, task: Task) -> Tuple[float, List[QualityIssue]]:
        ...
        if (
            "acceptance criteria" not in task.description.lower()
            and len(task.description) < 200
        ):
            issues.append(
                QualityIssue(
                    task_id=task.id,
    ...
    def _validate_labels(self, task: Task) -> Tuple[float, List[QualityIssue]]:
        ...
        label_categories = [
            label.split(":")[0] for label in task.labels if ":" in label
        ]
    ...
    def _calculate_overall_score(
        ...
        scores = {
            ...
            "labels": min(
                metrics["average_labels_per_task"] / self.MIN_LABELS_PER_TASK, 1.0
            ),
            "estimates": metrics["estimate_coverage"],
            "priorities": 1.0 if len(metrics["priority_distribution"]) > 1 else 0.5,
            "dependencies": min(
                metrics["dependency_coverage"] * 2, 1.0
            ),  # Less weight on 100% coverage
        }
