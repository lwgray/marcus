...
class MarcusMVP:
        ...
        async def handle_call_tool(
            ...
            try:
                if name == "register_agent":
                    ...
                    result = await self._request_next_task(arguments.get("agent_id"))
                    ...
                    result = await self._get_agent_status(arguments.get("agent_id"))

                elif name == "list_registered_agents":
                    result = await self._list_registered_agents()

                elif name == "ping":
                    result = await self._ping(arguments.get("echo", ""))
    ...
    async def _request_next_task(self, agent_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not available_tasks:
                return {"has_task": False, "message": "No tasks available at this time"}

            # MVP: Simple task selection (highest priority)
            priority_map = {"urgent": 4, "high": 3, "medium": 2, "low": 1}
            optimal_task = max(
                available_tasks, key=lambda t: priority_map.get(t.priority.value, 1)
            )
    ...
    async def _report_task_progress(
        self, agent_id: str, task_id: str, status: str, progress: int, message: str
    ) -> Dict[str, Any]:
        """
    ...
    async def _report_blocker(
        self, agent_id: str, task_id: str, blocker_description: str, severity: str
    ) -> Dict[str, Any]:
        """
...
This task is now blocked and requires attention."""
            ...
            resolution_suggestion = await self._get_basic_resolution(
                blocker_description
            )
    ...
    async def _get_agent_status(self, agent_id: str) -> Dict[str, Any]:
        ...
        try:
            if agent_id not in self.agent_status:
                return {"found": False, "message": f"Agent {agent_id} not registered"}
    ...
    async def _list_registered_agents(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "agent_count": len(agents), "agents": agents}
...
5. Document the resolution once complete"""
    ...
    async def _ping(self, echo: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            status = {
                ...
                "workload": {
                    ...
                    "total_completed_tasks": sum(
                        ...
                        for agent in self.agent_status.values()
                    ),
                    "agents_available": len(
                        [
                            a
                            for a in self.agent_status.values()
                            if a.worker_id not in self.agent_tasks
                        ]
                    ),
                },
            }
            ...
            return {"success": True, "pong": True, **status}
...
async def main() -> None:
    ...
    async with stdio_server() as (read_stream, write_stream):
        await agent.server.run(
            read_stream, write_stream, agent.server.create_initialization_options()
        )
