...
from .pipeline_flow import PipelineEvent, PipelineFlow, PipelineStage

PIPELINE_EVENTS_FILE = Path("logs/pipeline_events.json")
...
class SharedPipelineEvents:
    ...
    def add_event(self, flow_id: str, event: Dict[str, Any]):
        ...
        if "event_id" not in event_data:
            event_data["event_id"] = f"{flow_id}_{len([e for e in data['events'] if e.get('flow_id') == flow_id])}"
    ...
    def get_active_flows(self) -> List[Dict[str, Any]]:
        ...
        for flow_id, flow_data in data["flows"].items():
            ...
            if flow_data.get("is_active", False) or is_recent:
                ...
                event_count = sum(1 for e in data["events"] if e.get("flow_id") == flow_id)
...
class SharedPipelineVisualizer:
    ...
    def track_ai_analysis(self, flow_id: str, prd_text: str, analysis_result: Dict[str, Any], duration_ms: int):
        ...
        self.add_event(
            ...
            data={
                ...
                # Enhanced insights
                "extracted_requirements": analysis_result.get("extractedRequirements", []),
    ...
    def track_task_generation(self, flow_id: str, task_count: int, tasks: List[Dict[str, Any]], duration_ms: int, 
        ...
        self.add_event(
            ...
            data={
                ...
                # Enhanced insights
                "task_breakdown_reasoning": generation_context.get("reasoning", "") if generation_context else "",
                "dependency_graph": generation_context.get("dependencies", {}) if generation_context else {},
                "effort_estimates": generation_context.get("effort_estimates", {}) if generation_context else {},
                "risk_factors": generation_context.get("risk_factors", []) if generation_context else [],
                "alternative_structures": generation_context.get("alternatives_considered", []) if generation_context else [],
                "complexity_score": generation_context.get("complexity_score", 0) if generation_context else 0
            },
            duration_ms=duration_ms,
            status="completed"
        )
    ...
    def track_decision_point(self, flow_id: str, stage: PipelineStage, decision: str, 
                            rationale: str, confidence: float, alternatives: List[Dict[str, Any]]):
    ...
    def track_performance_metrics(self, flow_id: str, stage: PipelineStage, metrics: Dict[str, Any]):
