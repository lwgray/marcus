...
from src.core.error_framework import MarcusBaseError, ErrorContext
...
class TokenTracker:
        ...
        if self.data_file.exists():
            try:
                with open(self.data_file, 'r') as f:
                    data = json.load(f)
                    self.project_tokens = defaultdict(int, data.get('project_tokens', {}))
                    self.project_costs = defaultdict(float, data.get('project_costs', {}))
    ...
    def get_project_stats(self, project_id: str) -> Dict[str, any]:
        ...
        if project_id in self.session_start_times:
            duration = (datetime.now() - self.session_start_times[project_id]).total_seconds()
        else:
            duration = 0
    ...
    def get_all_projects_summary(self) -> Dict[str, any]:
        ...
        return {
            ...
            'active_projects': len([p for p in summary.values() if p['current_spend_rate'] > 0])
        }
            ...
            try:
                ...
                for project_id in self.project_tokens:
                    ...
                    if len(self.spend_rates[project_id]) > 60:
                        self.spend_rates[project_id] = self.spend_rates[project_id][-60:]
