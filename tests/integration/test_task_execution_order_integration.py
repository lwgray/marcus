...
class TestTaskExecutionOrderIntegration:
    ...
    @pytest.mark.asyncio
    async def test_complete_workflow_with_phase_dependencies(self, task_creator, kanban_client):
    ...
    async def test_multi_feature_project(self, kanban_client):
        ...
        async def multi_feature_process(description: str) -> List[Task]:
            return [
                ...
                creator._create_task("pay-2", "Implement payment processing", ["payment"]),
        ...
        for task in result:
            if task.dependencies:  # Only check tasks with dependencies
                ...
                for dep_id in task.dependencies:
                    ...
                    assert dep_task.labels[0] == task_feature, f"Cross-feature dependency: {task.name} ({task_feature}) depends on {dep_task.name} ({dep_task.labels[0]})"