...
class TestNLPBoardIntegration:
    ...
    async def test_project_appears_on_board(self):
        ...
        our_tasks = [
            ...
            if project_name in (task.title or "")
            or project_name in (task.description or "")
        ]

        # Verify tasks exist on board
        assert len(our_tasks) > 0, f"No tasks found on {self.provider} board"
        assert (
            len(our_tasks) == result["tasks_created"]
    ...
    async def test_feature_appears_on_board(self):
        ...
        twofa_tasks = [
            ...
            for task in all_tasks
            if (
                "two-factor" in task.title.lower()
                or "2fa" in task.title.lower()
                or "sms" in task.title.lower()
            )
        ]
