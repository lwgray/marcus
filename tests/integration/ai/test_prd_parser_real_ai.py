...
class TestPRDParserRealAI:
    ...
    async def test_simple_todo_api_real_response(self, parser, constraints):
        ...
        try:
            result = await parser.parse_prd_to_tasks(
                prd_content=prd_content, constraints=constraints
            )
            ...
            assert (
                len(task_types) >= 3
            ), f"Should have varied task types, got: {task_types}"

            # Verify no 'unknown' task IDs
            unknown_tasks = [task for task in result.tasks if "unknown" in task.id]
            assert (
                len(unknown_tasks) == 0
            ), f"Should not have 'unknown' task IDs: {[t.id for t in unknown_tasks]}"

            # Verify functional requirements were parsed
            func_req_tasks = [
                task
                for task in result.tasks
                if "crud" in task.id.lower() or "auth" in task.id.lower()
            ]
            assert (
                len(func_req_tasks) > 0
            ...
            crud_tasks = [
                ...
                if "crud" in t.name.lower() or "crud" in t.id.lower()
            ]
            auth_tasks = [
                t
                for t in result.tasks
                if "auth" in t.name.lower() or "auth" in t.id.lower()
            ]
            validation_tasks = [
                t
                for t in result.tasks
            ...
            assert len(crud_tasks) > 0 or any(
                "todo" in t.name.lower() or "todo" in t.id.lower() for t in result.tasks
            ), "Should have CRUD or todo-related tasks"
            assert len(auth_tasks) > 0 or any(
                "user" in t.name.lower() for t in result.tasks
    ...
    async def test_complex_ecommerce_real_response(self, parser, constraints):
        ...
        try:
            ...
            assert (
                len(result.tasks) >= 15
            ), f"Complex project should generate many tasks, got {len(result.tasks)}"
            ...
            ), f"Should cover multiple features, found: {found_features}"
    ...
    async def test_minimal_prd_real_response(self, parser, constraints):
        ...
        try:
            ...
            assert (
                len(result.tasks) >= 6
            ), "Should generate basic tasks even from minimal PRD"

            # Should have infrastructure tasks
            infra_tasks = [
                t
                for t in result.tasks
                if "setup" in t.name.lower() or "deploy" in t.name.lower()
            ]
    ...
    async def test_ai_response_format_consistency(self, parser, constraints):
        ...
        assert (
            field_usage["functional_reqs"].get("id", 0) > 0
            or field_usage["functional_reqs"].get("name", 0) > 0
    ...
    async def test_real_ai_deviation_logging(self, parser, constraints, caplog):
        ...
        try:
            ...
            deviation_logs = [
                ...
                if "deviated from template" in record.message
            ]
    ...
    async def test_task_name_specificity_real_ai(self, parser, constraints):
        ...
        try:
            ...
            for task in result.tasks:
                ...
                if any(
                    keyword in name_lower for keyword in generic_keywords
                ) and not any(keyword in name_lower for keyword in specific_keywords):
                    generic_count += 1
            ...
            specificity_ratio = (
                specific_count / len(result.tasks) if result.tasks else 0
            )
            assert (
                specificity_ratio >= 0.5
            ), f"At least 50% of tasks should have specific names, got {specificity_ratio:.1%}"
