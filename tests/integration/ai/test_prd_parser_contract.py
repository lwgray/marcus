...
from src.ai.advanced.prd.parser import AdvancedPRDParser
...
class TestPRDParserContract:
    ...
    def validate_functional_requirement(self, req: Dict[str, Any]) -> List[str]:
        ...
        for field in required_fields:
            if field not in req:
                violations.append(
                    f"Missing required field '{field}' in functional requirement"
                )
        ...
        if "description" in req and not isinstance(req["description"], str):
            violations.append(
                f"Field 'description' should be string, got {type(req['description'])}"
            )

        if "priority" in req and req["priority"] not in ["high", "medium", "low"]:
            violations.append(
                f"Field 'priority' should be 'high', 'medium', or 'low', got '{req.get('priority')}'"
            )
        ...
        if "id" in req:
            ...
            if " " in id_val or "-" in id_val or not id_val.replace("_", "").isalnum():
                violations.append(
                    f"ID should use underscore format (e.g., 'crud_operations'), got '{id_val}'"
                )
    ...
    def validate_nfr(self, nfr: Dict[str, Any]) -> List[str]:
        ...
        if "category" in nfr and nfr["category"] not in valid_categories:
            violations.append(
                f"NFR category should be one of {valid_categories}, got '{nfr.get('category')}'"
            )

        return violations

    def validate_prd_analysis_structure(
        ...
        if "implementationApproach" in analysis_data:
            ...
            if analysis_data["implementationApproach"] not in valid_approaches:
                violations.append(
                    f"implementationApproach should be one of {valid_approaches}"
                )
        ...
        if "complexityAssessment" in analysis_data:
            ...
            if not isinstance(ca, dict):
                ...
                for field in ["technical", "timeline", "resources"]:
                    if field not in ca:
                        violations.append(
                            f"complexityAssessment missing field '{field}'"
                        )
        ...
        if "confidence" in analysis_data:
            if not isinstance(analysis_data["confidence"], (int, float)):
                violations.append(
                    f"confidence should be a number, got {type(analysis_data['confidence'])}"
                )
            elif not 0 <= analysis_data["confidence"] <= 1:
                violations.append(
                    f"confidence should be between 0 and 1, got {analysis_data['confidence']}"
                )
        ...
        try:
            ...
            if raw_response:
                ...
                if violations:
                    ...
                    pytest.fail(
                        f"AI response violated contract with {len(violations)} violations"
                    )
                else:
            ...
            assert (
                len(analysis.functional_requirements) > 0
    ...
    async def test_contract_field_consistency(self, parser):
        ...
        field_consistency = {"functional_reqs": {}, "nfr_reqs": {}}
