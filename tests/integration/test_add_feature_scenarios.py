...
class TestAddFeatureScenarios:
    ...
    async def test_add_feature_with_empty_description(self, mock_state):
        ...
        with patch("nlp_tools.marcus_mcp_server.state", mock_state):
    ...
    async def test_add_feature_with_ai_engine(self, mock_state):
        ...
        mock_state.ai_engine.analyze_feature_request = AsyncMock(
            return_value={
                "required_tasks": [
                    {
        ...
        mock_state.ai_engine.analyze_integration_points = AsyncMock(
            return_value={"suggested_phase": "development", "confidence": 0.95}
        )
