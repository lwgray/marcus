...
"""
    ...
    create_result = await marcus_client.call_tool(
        "create_project", {"name": project_name, "description": prd_content}
    )
    ...
    register_result = await marcus_client.call_tool(
        "register_agent",
        {
    ...
    task_result = await marcus_client.call_tool(
        "request_next_task", {"agent_id": "test-agent-001"}
    )
    ...
    if task_result and "task" in task_result:
        ...
        progress_result = await marcus_client.call_tool(
