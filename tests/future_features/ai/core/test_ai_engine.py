...
class TestMarcusAIEngine:
    ...
    @pytest.mark.asyncio
    async def test_hybrid_analysis_rule_safety_preserved(
        self, ai_engine, analysis_context
    ):
        ...
        ai_engine.llm_client.analyze_task_semantics.return_value = Mock(
            confidence=0.9, recommendation="safe_to_deploy"
        )
    ...
    @pytest.mark.asyncio
    async def test_hybrid_analysis_ai_enhancement_when_safe(
        self, ai_engine, analysis_context
    ):
    ...
    async def test_fallback_when_ai_unavailable(self, ai_engine, analysis_context):
        ...
        ai_engine.rule_engine.analyze.return_value = Mock(
            is_valid=True, confidence=0.9, reason="Dependencies satisfied"
        )

        # Mock AI to fail
        ai_engine.llm_client.analyze_task_semantics.side_effect = Exception(
    ...
    async def test_confidence_weighting_logic(self, ai_engine, analysis_context):
        ...
        ai_engine.rule_engine.analyze.return_value = Mock(
            is_valid=True, confidence=0.95, reason="Strong rule match"
        )

        ai_engine.llm_client.analyze_task_semantics.return_value = Mock(
            confidence=0.3, reasoning="Uncertain semantic analysis"
        )
        ...
        assert (
            result.confidence_breakdown["rule_weight"]
            > result.confidence_breakdown["ai_weight"]
        )

    @pytest.mark.asyncio
    async def test_semantic_understanding_enhancement(
        self, ai_engine, analysis_context
    ):
        ...
        ai_engine.rule_engine.analyze.return_value = Mock(
            is_valid=True, confidence=0.6, reason="Basic keyword match"
        )
...
class TestLLMAbstraction:
    ...
    def llm_abstraction(self):
        ...
        with (
            patch(
                "src.ai.providers.llm_abstraction.AnthropicProvider"
            ) as mock_anthropic,
    ...
    async def test_provider_fallback(self, llm_abstraction):
        ...
        llm_abstraction.current_provider = "anthropic"
        llm_abstraction.providers["anthropic"].analyze_task.side_effect = Exception(
            "API rate limit"
        )
        llm_abstraction.providers["openai"].analyze_task.return_value = Mock(
...
class TestHybridDecisionFramework:
    ...
    def decision_framework(self):
        ...
        with (
            patch("src.ai.decisions.hybrid_framework.RuleBasedEngine") as mock_rules,
    ...
    async def test_ai_optimization_when_rules_pass(self, decision_framework):
        ...
        # Rules pass
        decision_framework.rule_engine.validate_assignment.return_value = Mock(
            is_valid=True, confidence=0.8, reason="All rule checks passed"
        )
    ...
    async def test_confidence_scoring_algorithm(self, decision_framework):
        ...
        decision_framework.rule_engine.validate_assignment.return_value = Mock(
            is_valid=True, confidence=0.6, reason="Partial rule match"
        )

        decision_framework.ai_engine.analyze_assignment_optimality.return_value = Mock(
            confidence=0.9, semantic_confidence=0.95
        )
