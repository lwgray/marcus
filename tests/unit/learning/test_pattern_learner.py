...
class TestPatternLearner:
    ...
    async def test_learn_from_project(self, pattern_learner, sample_project):
        ...
        pattern_learner._identify_failure_points = AsyncMock(return_value=[])
        ...
        pattern_learner._analyze_estimation_accuracy.assert_called_once_with(
            sample_project
        )
        pattern_learner._analyze_dependency_patterns.assert_called_once_with(
            sample_project
        )
        pattern_learner._analyze_workflow_patterns.assert_called_once_with(
            sample_project
        )
        pattern_learner._identify_success_factors.assert_called_once_with(
            sample_project
        )
        pattern_learner._identify_failure_points.assert_called_once_with(sample_project)
        pattern_learner._analyze_team_performance.assert_called_once_with(
            sample_project
        )
    ...
    async def test_identify_failure_points(self, pattern_learner, sample_project):
        ...
        if (
            len(
                [
                    t
                    for t in sample_project.tasks
                    if t.status.value in ["BLOCKED", "CANCELLED"]
                ]
            )
            > len(sample_project.tasks) * 0.1
        ):
    ...
    async def test_get_patterns_for_context(self, pattern_learner):
        ...
        pattern_learner.calculate_confidence = AsyncMock(
            side_effect=lambda p: p.confidence
        )
    ...
    async def test_complete_learning_workflow(self, pattern_learner, sample_project):
        ...
        for pattern_dict in export_data["patterns"].values():
            if "last_updated" in pattern_dict and isinstance(
                pattern_dict["last_updated"], datetime
            ):
