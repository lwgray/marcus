...
class TestRetryConfigSecurity:
    ...
    async def test_async_retry_uses_secure_random_for_jitter(
        self, mock_sleep, mock_system_random
    ):
...
class TestGracefulDegradation:
    ...
    async def test_graceful_degradation_context_manager(self):
        ...
        async with GracefulDegradation(
            primary=primary_func, fallback=fallback_func
        ) as gd:
    ...
    async def test_graceful_degradation_fallback_on_failure(self):
        ...
        async with GracefulDegradation(
            primary=failing_primary, fallback=fallback_func
        ) as gd:
            result = await gd.try_primary()
