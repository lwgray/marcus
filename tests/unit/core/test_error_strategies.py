...
class TestRetryHandler:
    ...
    @pytest.mark.parametrize(
        "side_effect,expected_result,expected_calls,should_raise",
        [
    ...
    async def test_retry_behavior(
        self, side_effect, expected_result, expected_calls, should_raise
    ):
    ...
    def test_delay_calculation_with_max(self):
        ...
        config = RetryConfig(
            base_delay=10.0, max_delay=15.0, multiplier=2.0, jitter=False
        )
    ...
    def test_should_stop_retry_logic(self):
        ...
        assert (
            self.handler._should_stop_retry(transient_error, 2) is True
        )  # attempt 2, max 3
        assert (
            self.handler._should_stop_retry(transient_error, 1) is False
        )  # attempt 1, max 3
...
class TestFallbackHandler:
    ...
    async def test_all_fallbacks_fail(self):
        ...
        assert (
            ...
            in exc_info.value.remediation.fallback_strategy
        )
...
class TestErrorAggregator:
    ...
    def test_add_error_marcus_error(self):
        ...
        assert (
            self.aggregator.errors[0].context.custom_context["item_context"]["item_id"]
            == "123"
        )
