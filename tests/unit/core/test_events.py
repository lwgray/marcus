...
class TestEvents:
    ...
    async def test_publish_event(self, events):
        ...
        event = await events.publish(
            event_type="test_event", source="test", data={"message": "hello"}
        )
    ...
    def test_get_history_filtering(self, events):
        ...
        async def run_test():
            ...
            type1_events = await events.get_history(event_type="type1")
            assert len(type1_events) == 2

            # Filter by source
            source1_events = await events.get_history(source="source1")
            assert len(source1_events) == 2

            # Filter by both
            filtered = await events.get_history(event_type="type1", source="source1")
