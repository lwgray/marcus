...
class TestEnhancedTaskClassifier:
    ...
    def create_task(self, id: str, name: str, description: str = "", labels=None, base_task_data=None, **kwargs):
    ...
    # Design task tests
    @pytest.mark.parametrize("name,description,expected_confidence", [
        ("Design authentication system", "Create flow diagrams and architecture", 0.9),
    ...
    def test_design_task_identification(self, classifier, name, description, expected_confidence):
        ...
        assert "design" in result.reasoning.lower() or "plan" in result.reasoning.lower()
    
    # Implementation task tests
    @pytest.mark.parametrize("name,description,expected_confidence", [
        ("Implement user authentication", "Build login and registration endpoints", 0.9),
        ("Create payment processing module", "Develop payment gateway integration", 0.85),
        ("Build REST API endpoints", "Code CRUD operations for resources", 0.9),
        ("Add feature for user profiles", "Implement profile management functionality", 0.85),
        ...
        ("Setup database connections", "Configure and establish DB connections", 0.8),
        ("Code algorithm for recommendations", "Implement recommendation engine", 0.9),
    ])
    def test_implementation_task_identification(self, classifier, name, description, expected_confidence):
    ...
    def test_testing_task_identification(self, classifier, name, description, expected_confidence):
    ...
    def test_documentation_task_identification(self, classifier, name, description, expected_confidence):
    ...
    def test_deployment_task_identification(self, classifier):
        ...
        task = self.create_task(
            ...
            "Release version 2.0 to live environment"
        )
        result = classifier.classify_with_confidence(task)
    ...
    def test_no_suggestions_for_clear_task(self, classifier):
        ...
        task = self.create_task(
            ...
            "Create detailed architecture for auth"
        )
        suggestions = classifier.get_suggestions(task)
