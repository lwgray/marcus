...
class TestModeRegistry:
    ...
    def mode_registry(self, mock_creator_mode, mock_adaptive_mode):
        ...
        with patch(
            "src.modes.creator.basic_creator.BasicCreatorMode",
            return_value=mock_creator_mode,
        ):
            with patch(
                "src.modes.adaptive.basic_adaptive.BasicAdaptiveMode",
                return_value=mock_adaptive_mode,
            ):
    ...
    def test_get_available_modes(self, mode_registry):
        ...
        assert available == {"creator": True, "enricher": False, "adaptive": True}
    ...
    async def test_get_mode_history_with_switches(self, mode_registry):
        ...
        await mode_registry.switch_mode(
            MarcusMode.CREATOR, reason="Switch 1", user_id="user1"
        )
        await mode_registry.switch_mode(
            MarcusMode.ADAPTIVE, reason="Switch 2", user_id="user2"
        )
    ...
    async def test_state_isolation_between_modes(self, mode_registry):
        ...
        mode_registry.modes[MarcusMode.CREATOR].get_state = AsyncMock(
            return_value=creator_state
        )
        mode_registry.modes[MarcusMode.ADAPTIVE].get_state = AsyncMock(
            return_value=adaptive_state
        )
