...
class MockTask:
    """
    Mock Task class that matches the real Task model structure.

    This mock allows us to test the actual behavior using the same fields
    as the real Task model, including source_context.
    """
...
def basic_creator_mode(mock_templates, mock_task_generator):
    ...
    with (
        patch(
            "src.modes.creator.basic_creator.WebAppTemplate",
            return_value=mock_templates["web"],
        ),
        patch(
            "src.modes.creator.basic_creator.APIServiceTemplate",
            return_value=mock_templates["api"],
        ),
        patch(
            "src.modes.creator.basic_creator.MobileAppTemplate",
            return_value=mock_templates["mobile"],
        ),
        patch(
            "src.modes.creator.basic_creator.TaskGenerator",
...
class TestProjectCreationFromTemplate:
    ...
    async def test_create_project_success(self, basic_creator_mode):
        ...
        mock_tasks = [
            MockTask(
                ...
                phase="Planning",
                estimated_hours=2,
                source_context={"phase": "Planning", "phase_order": 1},
            ),
            MockTask(
                id="task-2",
                ...
                phase="Development",
                estimated_hours=4,
                source_context={"phase": "Development", "phase_order": 2},
            ),
        ]
    ...
    async def test_create_project_invalid_template(self, basic_creator_mode):
        ...
        result = await basic_creator_mode.create_project_from_template(
            template_name="invalid", project_name="Test Project"
        )
    ...
    async def test_create_project_with_exception(self, basic_creator_mode):
        ...
        basic_creator_mode.task_generator.generate_from_template.side_effect = (
            Exception("Generator error")
        )

        result = await basic_creator_mode.create_project_from_template(
            template_name="web", project_name="Test Project"
        )
    ...
    async def test_create_project_without_customizations(self, basic_creator_mode):
        ...
        result = await basic_creator_mode.create_project_from_template(
            template_name="api", project_name="API Service"
        )
...
class TestProjectCustomization:
    ...
    async def test_customize_project_success(self, basic_creator_mode):
        ...
        mock_tasks = [MockTask(id="task-1", name="New task", estimated_hours=3)]
        basic_creator_mode.task_generator.generate_from_template.return_value = (
            mock_tasks
        )
        ...
        updated_customizations = basic_creator_mode.state["active_project"][
            "customizations"
        ]
    ...
    async def test_get_available_templates(self, basic_creator_mode):
        ...
        for name, template in basic_creator_mode.templates.items():
            ...
            template.get_all_tasks.side_effect = lambda size=None: [
                ...
                Mock(estimated_hours=3),
                Mock(estimated_hours=4),
                Mock(estimated_hours=1),
                Mock(estimated_hours=2),
            ]
...
class TestCreateFromDescription:
    ...
    async def test_create_from_description_mobile_keywords(self, basic_creator_mode):
        ...
        result = await basic_creator_mode.create_from_description(
            "Build an iOS app for task management", "TaskMaster"
        )

        # Verify mobile template was selected
        call_args = basic_creator_mode.task_generator.generate_from_template.call_args
        assert call_args.kwargs["template"] == basic_creator_mode.templates["mobile"]
        assert result["suggestion_reasoning"].startswith(
            "Based on your description, I suggested a mobile"
        )
    ...
    async def test_create_from_description_api_keywords(self, basic_creator_mode):
        ...
        result = await basic_creator_mode.create_from_description(
            "Create a REST API service for user management", "UserAPI"
        )
    ...
    async def test_create_from_description_web_keywords(self, basic_creator_mode):
        ...
        result = await basic_creator_mode.create_from_description(
            "Develop a full-stack website with React", "MyWebsite"
        )
    ...
    async def test_create_from_description_mvp_size(self, basic_creator_mode):
        ...
        result = await basic_creator_mode.create_from_description(
            "Quick prototype for a mobile app", "Prototype"
        )
    ...
    async def test_create_from_description_large_size(self, basic_creator_mode):
        ...
        result = await basic_creator_mode.create_from_description(
            "Enterprise-level complex system", "EnterpriseApp"
        )
    ...
    async def test_create_from_description_default_template(self, basic_creator_mode):
        ...
        result = await basic_creator_mode.create_from_description(
            "Build something cool", "CoolProject"
        )
...
class TestInternalMethods:
    ...
    def test_task_to_dict_missing_metadata(self, basic_creator_mode):
        """Test task_to_dict handles missing source_context gracefully."""
        task = MockTask(source_context={})
    ...
    def test_get_phases_summary(self, basic_creator_mode):
        ...
        tasks = [
            ...
            MockTask(
                name="Task 4",
                estimated_hours=1,
                source_context={"phase": "Testing", "phase_order": 3},
            ),
        ]
...
class TestRealTaskCompatibility:
    """Test suite to verify compatibility with real Task objects."""

    def test_mock_task_compatible_with_real_task(self, basic_creator_mode):
        """Test that MockTask behaves the same as real Task for _task_to_dict."""
        # Create a real Task object
        real_task = Task(
            id="real-task-001",
            name="Real Task",
            description="A real task for testing",
        ...
        assert real_dict["phase_order"] == mock_dict["phase_order"] == 1
        assert real_dict["generated"] == mock_dict["generated"] is True
...
class TestEdgeCases:
    ...
    async def test_empty_generated_tasks(self, basic_creator_mode):
        ...
        result = await basic_creator_mode.create_project_from_template(
            template_name="web", project_name="Empty Project"
        )
    ...
    async def test_concurrent_customization_calls(self, basic_creator_mode):
        ...
        result1 = await basic_creator_mode.customize_project(
        ...
        final_customizations = basic_creator_mode.state["active_project"][
            "customizations"
        ]
