...
def sample_features() -> List[Feature]:
    ...
    return [
        ...
        Feature(
            ...
            priority="low",
            user_stories=["As an admin, I want to see basic metrics"],
            acceptance_criteria=[
                "Dashboard shows user count",
                "Dashboard shows recent activity",
            ],
            technical_notes=["Simple read-only interface"],
...
class TestTaskTemplateCustomization:
    ...
    def test_customize_template_for_feature(self, task_generator, sample_features):
        ...
        context = ProjectContext(
            tech_stack=Mock(), team_size=3, timeline_weeks=12, complexity_level="medium"
        )

        customized = task_generator._customize_template_for_feature(
...
class TestTaskGeneration:
    ...
    async def test_generate_design_tasks(self, task_generator, sample_features):
        ...
        assert any(
            "wireframes" in name.lower() for name in task_names
        )  # Frontend included
    ...
    @pytest.mark.asyncio
    async def test_generate_design_tasks_no_frontend(
        self, task_generator, sample_features
    ):
        """
    ...
    @pytest.mark.asyncio
    async def test_generate_feature_tasks(
        self, task_generator, sample_features, sample_tech_stack
    ):
        """
    ...
    @pytest.mark.asyncio
    async def test_generate_tasks_for_specific_feature(
        self, task_generator, sample_features, sample_tech_stack
    ):
        """
        ...
        auth_template_names = [
            t["name"]
            for t in task_generator.feature_task_templates["user_authentication"]
        ]
    ...
    async def test_generate_integration_tasks(self, task_generator, sample_features):
        ...
        tasks = await task_generator._generate_integration_tasks(
    ...
    @pytest.mark.asyncio
    async def test_generate_testing_tasks(
        self, task_generator, sample_features, sample_tech_stack
    ):
        """
        ...
        assert any(
            "fix" in name.lower() and "bugs" in name.lower() for name in task_names
        )
    ...
    async def test_generate_deployment_tasks(self, task_generator, sample_tech_stack):
        ...
        tasks = await task_generator._generate_deployment_tasks(
            sample_tech_stack, context
        )
...
class TestFullProjectGeneration:
    ...
    async def test_generate_tasks_from_prd(self, task_generator, sample_prd):
        ...
        with patch.object(
            task_generator,
            "_generate_setup_tasks",
            return_value=[create_mock_task("setup-1", "Setup Task")],
        ):
            with patch.object(
                task_generator,
                "_generate_design_tasks",
                return_value=[create_mock_task("design-1", "Design Task")],
            ):
                with patch.object(
                    task_generator,
                    "_generate_feature_tasks",
                    return_value=[create_mock_task("feature-1", "Feature Task")],
                ):
                    with patch.object(
                        task_generator,
                        "_generate_integration_tasks",
                        return_value=[
                            create_mock_task("integration-1", "Integration Task")
                        ],
                    ):
                        with patch.object(
                            task_generator,
                            "_generate_testing_tasks",
                            return_value=[
                                create_mock_task("testing-1", "Testing Task")
                            ],
                        ):
                            with patch.object(
                                task_generator,
                                ...
                                return_value=[
                                    create_mock_task("deployment-1", "Deployment Task")
                                ],
                            ):

                                project_structure = (
                                    await task_generator.generate_tasks_from_prd(
                                        sample_prd
                                    )
                                )
    ...
    async def test_generate_tasks_from_prd_integration(
        self, task_generator, sample_prd
    ):
        """
