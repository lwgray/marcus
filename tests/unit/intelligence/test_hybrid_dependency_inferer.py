...
class TestHybridDependencyInferer:
    ...
    @pytest.mark.asyncio
    async def test_hybrid_mode_high_confidence_patterns(
        self, mock_ai_engine, test_tasks
    ):
        """Test that high confidence patterns don't trigger AI"""
        config = HybridInferenceConfig(
            pattern_confidence_threshold=0.8, enable_ai_inference=True
        )
    ...
    @pytest.mark.asyncio
    async def test_hybrid_mode_low_confidence_triggers_ai(
        self, mock_ai_engine, test_tasks
    ):
        """Test that low confidence patterns trigger AI analysis"""
        # Set very high pattern threshold so AI is triggered
        config = HybridInferenceConfig(
            pattern_confidence_threshold=0.99, enable_ai_inference=True  # Very high
        )
    ...
    async def test_combined_confidence_boost(self, mock_ai_engine, test_tasks):
        ...
        config = HybridInferenceConfig(
            pattern_confidence_threshold=0.7, combined_confidence_boost=0.15
        )
    ...
    async def test_cache_functionality(self, mock_ai_engine, test_tasks):
        ...
        config = HybridInferenceConfig(
            pattern_confidence_threshold=0.99, cache_ttl_hours=24  # Force AI
        )
    ...
    async def test_min_shared_keywords_config(self, mock_ai_engine):
        ...
        tasks = [
            ...
            Task(
                id="2", name="Product API", **self._task_defaults()
            ),  # Only shares "API"
        ]

        # With min_shared_keywords=2, these shouldn't be considered related
        config = HybridInferenceConfig(min_shared_keywords=2, enable_ai_inference=True)
        inferer = HybridDependencyInferer(mock_ai_engine, config)
