...
class TestRunUsesRecommendedAgentCount:
    ...
    def test_run_caps_at_user_count_when_recommended_higher(
        self, spawner_with_config: Any, tmp_path: Path
    ) -> None:
        """CPM recommends 4, user cap is 2 → spawn min(4,2)=2 workers."""
        spawned = self._run_with_recommended(spawner_with_config, recommended=4)
        assert len(spawned) == 2, f"Expected 2 workers (user cap), got {len(spawned)}"

    def test_run_uses_recommended_when_lower_than_cap(
        self, spawner_with_config: Any, tmp_path: Path
    ) -> None:
        """CPM recommends 1, user cap is 2 → spawn min(1,2)=1 worker."""
        spawned = self._run_with_recommended(spawner_with_config, recommended=1)
        assert (
            len(spawned) == 1
        ), f"Expected 1 worker (recommended < cap), got {len(spawned)}"

    def test_run_uses_cap_when_recommended_equals_cap(
        self, spawner_with_config: Any, tmp_path: Path
    ) -> None:
        """CPM recommends 2, user cap is 2 → spawn min(2,2)=2 workers."""
        spawned = self._run_with_recommended(spawner_with_config, recommended=2)
