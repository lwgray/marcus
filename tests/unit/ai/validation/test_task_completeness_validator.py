...
class TestTaskCompletenessValidator:
    ...
    async def test_validate_coverage_complete(
        ...
        with patch.object(
            validator,
            "_call_ai",
            return_value={"complete": True, "missing": []},
        ):
            # Act
    ...
    async def test_validate_coverage_incomplete(
        ...
        with patch.object(
            validator,
            "_call_ai",
            return_value={"complete": False, "missing": ["MCP server"]},
        ):
            # Act
            result = await validator.validate_coverage(intents, sample_tasks)
    ...
    async def test_validate_with_retry_passes_first_attempt(
        ...
        async def mock_call_ai(prompt: str) -> dict:
            call_count[0] += 1
            if call_count[0] == 1:
                # extract_intents call (flat format for backwards compat)
                return {"intents": ["MCP tools", "API client"]}
            else:
    ...
    async def test_validate_with_retry_passes_second_attempt(
        ...
        async def mock_call_ai(prompt: str) -> dict:
            ...
            if call_count[0] == 1:
                # extract_intents call (flat format for backwards compat)
                return {"intents": ["MCP server", "MCP tools", "API client"]}
            elif call_count[0] == 2:
                # First validate_coverage call (incomplete)
                return {"complete": False, "missing": ["MCP server"]}
            else:
    ...
    async def test_validate_with_retry_fails_after_max_attempts(
        ...
        async def mock_call_ai(prompt: str) -> dict:
            ...
            if call_count[0] == 1:
                # extract_intents call (flat format for backwards compat)
                return {"intents": ["MCP server", "MCP tools"]}
            else:
