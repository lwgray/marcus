...
from src.core.models import ProjectState, RiskLevel, Task, TaskStatus, WorkerStatus
...
class TestHealthMonitor:
    ...
    async def test_get_project_health_success(
        self, health_monitor, mock_ai_engine, sample_project_state, sample_team_status
    ):
    ...
    def test_get_health_summary_with_data(self, health_monitor):
        ...
        health_monitor.analysis_history = [
            {"overall_health": "green", "risk_factors": []},
            {
                "overall_health": "yellow",
                "risk_factors": [{"severity": "high"}, {"severity": "medium"}],
            },
            {"overall_health": "green", "risk_factors": [{"severity": "low"}]},
            {
                "overall_health": "red",
                "risk_factors": [
                    {"severity": "high"},
    ...
    async def test_history_limit(self, health_monitor, mock_ai_engine):
        ...
        # Add 105 analyses
        for _ in range(105):
            await health_monitor.get_project_health(Mock(), [], [])
