...
class TestFireRunCostFeatures:
    """``fire_run_cost_features`` — the cost-model training-row event (#546)."""

    def _full_features(self) -> dict:
        """A representative full feature vector + cost outcome."""
        return {
            "domain": "fintech",
    ...
    def test_event_name_and_full_payload(
        self, isolated_home: Path, mock_client: Any
    ) -> None:
        """All allowlisted feature keys round-trip onto the event."""
        from src.telemetry.events import (
            _RUN_COST_FEATURE_KEYS,
        ...
        args, _ = mock_client.capture.call_args
        assert args[0] == "run_cost_features"
        # Every supplied key is allowlisted and survives.
        assert set(args[1].keys()) == _RUN_COST_FEATURE_KEYS
        assert args[1]["cost_usd_cents"] == 47
        assert args[1]["detected_tech_stack"] == ["python", "react"]

    def test_non_allowlisted_keys_dropped(
        self, isolated_home: Path, mock_client: Any
    ) -> None:
        """Identifying / unknown keys never reach the wire.

        Falsification recipe: remove the allowlist filter in
        ``fire_run_cost_features``. Confirm this test fails because
        ``run_id`` / ``project_name`` leak into the payload.
        """
        from src.telemetry.events import fire_run_cost_features

        polluted = self._full_features()
        polluted["run_id"] = "SECRET-run-uuid"
        polluted["project_name"] = "ProjectX"
        polluted["raw_prd_text"] = "the whole PRD"

        fire_run_cost_features(polluted)
        props = mock_client.capture.call_args[0][1]
        assert "run_id" not in props
        assert "project_name" not in props
        assert "raw_prd_text" not in props
        flat = " ".join(str(v) for v in props.values())
        assert "SECRET" not in flat
        assert "ProjectX" not in flat

    def test_partial_features_ship_what_is_present(
        self, isolated_home: Path, mock_client: Any
    ) -> None:
        """A sparse feature dict ships only the keys it has."""
        from src.telemetry.events import fire_run_cost_features

        fire_run_cost_features({"domain": "gaming", "cost_usd_cents": 10})
        props = mock_client.capture.call_args[0][1]
        assert props == {"domain": "gaming", "cost_usd_cents": 10}
    ...
    def test_swallows_exceptions(
        ...
        broken.capture.side_effect = RuntimeError("simulated")
        monkeypatch.setattr("src.telemetry.events.get_telemetry_client", lambda: broken)
        fire_run_cost_features({"domain": "x"})  # Must not raise.
