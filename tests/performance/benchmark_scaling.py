...
class AgentSimulator:
    ...
    It is designed to stress test the Marcus server under various load conditions.
...
class LoadTester:
    ...
    async def run_scenario(
        ...
        logger.info(
            f"Agents: {num_agents}, Duration: {duration}s, Type: {connection_type}"
        )
        ...
        result = BenchmarkResult(
            ...
            connections_per_second=(
                len(agents) / ramp_up_time if ramp_up_time > 0 else 0
            ),
...
async def main():
    ...
    parser.add_argument(
        "--connection-limit-test", action="store_true", help="Test connection limits"
    )
