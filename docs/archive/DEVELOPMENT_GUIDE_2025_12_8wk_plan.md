...
**Updated Schedule**: This guide originally followed an 8-week implementation
plan focused on telemetry and a unified dashboard. As of 2026-04, much has
shipped, some plans have been overtaken by reality, and the dashboard
bundling work is still pending. See the **2026-04 reality check** below
before following any week-by-week instructions.

> ## ⚠ 2026-04 Reality Check
>
> | Plan item | Status | Notes |
> |---|---|---|
> | **Week 1 — Configuration system** | ✅ **SHIPPED** | `src/config/marcus_config.py` + `src/config/settings.py`. CLAUDE_API_KEY env var added (commit 4e3014ce). |
> | **Week 2 — Telemetry & CATO API** | 🟡 **PARTIAL** | MCP tools exist; the planned REST endpoints (`/api/cato/snapshot`, `/api/cato/events/stream`, terminal SSE, TerminalManager/PTY) are **not built**. |
> | **Week 3 — Production validations** | ✅ **SHIPPED (different shape)** | Task/project/dependency validation is production-grade and recently hardened (#421). It is *not* the seven discrete `#118-125` items the original plan named. |
> | **Week 3 — Docker** | ✅ Shipped | `docker-compose.yml` is the infrastructure-only Planka + Postgres shape per #202. |
> | **Week 4 — Cato git submodule** | ❌ **NOT STARTED** | No `.gitmodules`, no `src/dashboard/`. Cato remains a sibling repo at `~/dev/cato`. |
> | **Week 5 — Unified install** | ❌ Not started | No post-install hook for Cato frontend build. |
> | **Week 6 — Unified `marcus start`** | 🟡 Partial | `marcus` CLI runs the MCP server only; it does not orchestrate Cato or the dashboard. |
> | **Week 7 — 6-tab dashboard** | 🟡 **EXTERNALIZED** to Cato | Cato shipped Live + Historical + a Quality dashboard (commit 5f8e54d in Cato). Launch / Terminals / Global tabs are not in either repo yet. |
> | **Week 8 — Web Console** | ❌ Not started | No xterm.js console, no PTY recovery. |
>
> **Big things shipped that aren't in this guide:**
> - **Parallel multi-instance experiments** (commit 0da35ed3) — per-instance
>   SQLite kanban DBs, env-var-driven MCP URLs, spawn-time race fixes. This
>   is the v0.4.0 work currently on `develop`.
> - **Epictetus coordination-effectiveness audit** (#263) — automated
>   evaluation of agent performance, surfaced in Cato's Quality dashboard.
> - **Validation hardening** (#421, #337) — citation-backed validation,
>   hallucination elimination, retry ceilings.
> - **Coordination/correctness decoupling** (commit 4ef01f3a).
> - **Coordination tax research infrastructure** (commit 2801ea6d) —
>   24-PRD test suite for NeurIPS 2026 submission.
>
> **What this means for the plan below:**
> 1. Treat Weeks 1–3 as essentially done; revisit only Week 2 telemetry if
>    a real consumer needs it.
> 2. Weeks 4–7 (Cato bundling) are the **real next milestone** but need
>    fresh design — Cato has evolved independently and the alignment
>    document (`MVP_CATO_ALIGNMENT_EVALUATION.md`) is now stale.
> 3. Week 8 (Web Console) is overlapping with the proposed Marcus Studio
>    desktop app (issue #443) and may be subsumed into that workstream.
>
> ## 2026-04 Triage Decisions
>
> After walking the open items individually, the following decisions
> have been recorded (cascading decisions inherited from
> `MVP_CATO_ALIGNMENT_EVALUATION.md` are also listed for completeness):
>
> | Item | Decision | Notes |
> |---|---|---|
> | **Week 1 — Configuration polish** (last 20%) | **KEEP** | Finish the type-safe dataclass migration + example template. Small remaining surface. |
> | **Week 2 — `UserJourneyTracker` + `ResearchEventLogger`** | **KEEP** | Build the missing user-journey tracker; research event infrastructure already landed (commit 2801ea6d). |
> | **Week 2 — `/api/cato/*` REST endpoints** | DEFER (inherited) | Cato continues reading SQLite + Planka directly. Rebuild only when Marcus Studio (#443) needs it. |
> | **Week 2 — Launch tab REST APIs** | DROP (inherited) | MCP-only is the contract. No REST wrappers. |
> | **Week 2 — Terminal streaming + `TerminalManager`** | DROP (inherited) | Marcus core does not own terminals. Terminals stay in Posidonius + tmux. |
> | **Week 3 — Validations + Docker + pip packaging** | **DONE** | Validation hardened (#421, #337). Docker is infrastructure-only (#202). Pip + `marcus` CLI ship. Closing this week. |
> | **Week 4 — Cato git submodule** | DROP (inherited) | Cato remains a sibling product. No `src/dashboard/` integration. |
> | **Week 5 — Unified `pip install marcus`** | DROP (inherited, depends on Week 4) | |
> | **Week 6 — Unified `marcus start` (launch Cato too)** | DROP (inherited, depends on Weeks 4-5) | `marcus start` continues launching Marcus MCP server only. |
> | **Week 7 — Launch tab UI** | DEFER (inherited) | Backend dropped; revisit at Marcus Studio M0 (#443). |
> | **Week 7 — Terminals tab UI** | DEFER (inherited) | Backend dropped; revisit at Marcus Studio M0 (#443). |
> | **Week 7 — Kanban tab REST proxy** | DROP (inherited) | Planka iframe stays the model. |
> | **Week 7 — Live tab UI** | **DONE** | Shipped in Cato (commit 5f8e54d). Track future work in the Cato repo, not here. |
> | **Week 7 — Historical tab UI** | **DONE** | Shipped in Cato. Track future work in the Cato repo, not here. |
> | **Week 7 — Global tab + `/api/cato/global/metrics`** | KEEP (inherited 1.8) | Single surviving "build it" item from the original alignment doc. |
> | **Week 8 — Web Console (xterm.js + PTY recovery)** | **DROP** | Posidonius (sibling repo) covers this surface. The web console is never built; experiment terminals stay in Posidonius / tmux / `/marcus` skill. |
> | **Phase 4 — Progressive Feedback UI** | **DEFER** | Loading indicators for slow project creation. Useful but not blocking; revisit when there's a real complaint. |
> | **Phase 4 — UX Improvements** (tooltips, keyboard shortcuts, theme, a11y) | **DEFER** | Belongs on Cato's roadmap, not Marcus core. Not blocking. |
>
> **What's actually live as a result of this triage:**
> 1. Finish Week 1 config polish (small)
> 2. Build the `UserJourneyTracker` (small)
> 3. Build the Global tab aggregation endpoint when someone has time
>
> **Everything else is either done, dropped, or punted to Marcus Studio (#443).**
>
> ## ⚠ This document is now archived
>
> Following the 2026-04 triage, the original 8-week plan no longer
> reflects what's actually live work. This file remains as a historical
> record of the December 2025 plan. The current developer-facing guide
> lives at the same path on `develop` — see the top of `DEVELOPMENT_GUIDE.md`
> for the active onboarding instructions.
>
> The historical version of this file is preserved at
> `docs/archive/DEVELOPMENT_GUIDE_2025_12_8wk_plan.md`.
