...
""".format(
        total_files=sum(len(mods) for mods in modules_by_package.values())
    )
    ...
    package_titles = {
        ...
        "recommendations": (
            "Recommendations",
            "Task and optimization recommendations.",
        ),
        "workflow": ("Workflow", "Workflow management and state machines."),
        "modes": (
            "Operational Modes",
            "Different operational modes (adaptive, creator, enricher).",
        ),
        "config": ("Configuration", "Configuration management and settings."),
        "infrastructure": (
            "Infrastructure",
            "Event bus, service registry, persistence.",
        ),
    ...
    for package_name in package_order:
        ...
        title, description = package_titles.get(
    ...
    print(
        f"Total modules documented: {sum(len(mods) for mods in modules_by_package.values())}"
    )
