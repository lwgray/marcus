...
def create_test_project():
    ...
    response = requests.post(f"{API_BASE_URL}/projects/create", json={
...
def add_features(project_id):
    ...
    for feature in features:
        response = requests.post(f"{API_BASE_URL}/projects/features/add", json={
...
def start_workflow(project_id):
    ...
    response = requests.post(f"{API_BASE_URL}/projects/workflow/start", json={
        "project_id": project_id,
        "options": {
            "auto_assign": True,
            ...
            if response.status_code == 200:
                ...
                if data['success']:
                    ...
                    if our_flow:
                        ...
                        print(f"  Health: {our_flow.get('health_status', {}).get('status', 'Unknown')}")
                        print()
